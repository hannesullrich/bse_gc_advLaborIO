function [emaxs] = backwards_induction_3d_vect(specs, states, indexer, log_wage_systematic, non_consumption_utility, draws_emax)
...
%%%%%%%%%%%%%%%%%%%%%%
    ...
    log_wage_systematic_period = log_wage_systematic(idx_states_period);
        log_wage_systematic_period = reshape(log_wage_systematic_period, 1, 1, length(idx_states_period));
        log_wage_systematic_period = repmat(log_wage_systematic_period, num_draws, 1, 1);
    
    % get non-consumption utility component
    non_consumption_utility_period = non_consumption_utility(idx_states_period, :);
        non_consumption_utility_period = permute(non_consumption_utility_period, [2, 1]);
        non_consumption_utility_period = reshape(non_consumption_utility_period, [1, num_choices, length(idx_states_period)]);
        non_consumption_utility_period = repmat(non_consumption_utility_period, num_draws, 1, 1);
    ...
    continuation_values_period = emaxs(idx_states_period, 1:num_choices);
        continuation_values_period = permute(continuation_values_period, [2, 1]);
        continuation_values_period = reshape(continuation_values_period, 1, num_choices, length(idx_states_period));
        continuation_values_period = repmat(continuation_values_period, num_draws, 1, 1);   
    ...
    emax_period = max(value_functions_period, [], 2);
        % sum over draws
        emax_period = sum(emax_period, 1);
        emax_period = emax_period / num_draws;
    
    % Write maximized value function to EMAXS matrix
    emax_period = reshape(emax_period, length(idx_states_period), 1, 1);
    
    emaxs(idx_states_period, 4) = emax_period;
    %%%%%%%%%%%%%%%%%%%%%%%
